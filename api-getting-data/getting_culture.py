...
# The API returns a paginated response with a 'content' list
# Each character has: uuid, first_name, last_name, nicknames, status, etc.
# NEW - works with a plain list response
...
# If no results, fall back to a broader Stranger Things related search
...
# Filter out the API key from results before saving
# The Europeana response includes 'apikey' at the top level — remove it
